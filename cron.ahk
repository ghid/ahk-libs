class Cron {
    ...
    buildExpression(current_min) {
        ...
        nthWeekDay := Cron.nthWeekdayOfMonth()
        return "\n(?P<Number>\d+?):\s*"
    ...
    pattern() {
        return "SO)^"
                ...
                . Format(Cron.patternString, "weekday", "[1-7]#[1-5]|")
                . "(?P<functionName>.+?)$"
    }
    ...
    nthWeekdayOfMonth(useTimestamp="", useWeekday="", useDay="") {
        timeStamp := new Calendar(useTimestamp)
        weekday := (useWeekday == "" ? A_WDay : useWeekday)
        day := (useDay == "" ? A_DD : useDay)
        loop 5 {
            if (timeStamp.findWeekDay(weekday, A_Index).asDay() == day) {
                return "|" weekday "#" A_Index
            }
        }
    }
}
